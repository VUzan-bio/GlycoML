...
MONOSAC_TYPES = [
    "Glc", "GlcNAc", "Gal", "GalNAc", "Man", "ManNAc", "Fuc",
    "Neu5Ac", "Neu5Gc", "KDN", "Kdo", "Xyl",
    "GlcA", "GalA", "IdoA", "Rha", "Ara",
]
...
_MONOSAC_ALIASES = {"NeuAc": "Neu5Ac", "NeuGc": "Neu5Gc", "Sia": "Neu5Ac"}

# Longest-first alternation so that "GalNAc" matches before "Gal" can swallow
# the prefix. Anomer letters (lowercase a/b for alpha/beta) are NOT treated as
# token boundaries because IUPAC condensed notation places them immediately
# after a residue name; longest-first alternation handles disambiguation.
_GLYCAN_TOKENS = sorted(
    set(MONOSAC_TYPES) | set(_MONOSAC_ALIASES.keys()),
    key=lambda name: (-len(name), name),
)
GLYCAN_TOKEN_RE = re.compile(
    "(" + "|".join(re.escape(t) for t in _GLYCAN_TOKENS) + ")"
)
...
def tokenize_iupac(iupac: str) -> List[str]:
    ...
    Aliases (NeuAc, NeuGc, Sia) are normalised to canonical Neu5Ac/Neu5Gc.
    """
    if not iupac:
        return []
    return [_MONOSAC_ALIASES.get(t, t) for t in GLYCAN_TOKEN_RE.findall(iupac)]
...
def _rdkit_fingerprint(smiles: str, radius: int, n_bits: int) -> torch.Tensor:
    """Morgan (ECFP-style) fingerprint sensitive to glycan stereochemistry.
    ...
    different stereocentres at atom resolution. Caller must provide isomeric
    SMILES (RDKit ``MolToSmiles(mol, isomericSmiles=True)`` or
    glycowork/glypy-generated SMILES).
    """
...
def count_monosaccharides(iupac: Optional[str]) -> List[float]:
    """Per-residue counts using a longest-first IUPAC tokeniser.

    Replaces the previous ``str.count``-based implementation, which inflated
    counts for any token that was a substring of a longer one (e.g. ``Gal``
    inside ``GalNAc``, ``Ac`` inside ``Neu5Ac``).
    """
    if not iupac:
        return [0.0 for _ in MONOSAC_TYPES]
    tokens = tokenize_iupac(iupac)
    return [float(tokens.count(name)) for name in MONOSAC_TYPES]
