...
class ESM2Embedder:
    ...
    def _discover_lora_target_modules(self) -> List[str]:
        ...
        ``q_proj``, ``k_proj``, ``v_proj`` and ``out_proj`` as ``nn.Linear``
        children. HuggingFace ESM-2 ports use the same names inside
        ``EsmSelfAttention``. We walk the live module tree and keep only
        names that actually exist to avoid PEFT silently attaching zero
        adapters.
        """
        ...
        for name, module in self.model.named_modules():
            ...
            if leaf in self._LORA_CANDIDATE_PROJECTIONS and isinstance(module, nn.Linear):
                present.add(leaf)
        # Hu et al. 2021 recommend adapting Q and V only.
        preferred = [p for p in ("q_proj", "v_proj") if p in present]
        return preferred if preferred else sorted(present)

    def setup_lora(self, lora_rank: int = 8, lora_alpha: int = 32, lora_dropout: float = 0.1) -> None:
        """Wrap the underlying ESM-2 with LoRA adapters for fine-tuning.

        ESM-2 is a masked (BERT-style) encoder (Lin et al. 2023). Uses
        ``TaskType.FEATURE_EXTRACTION`` so PEFT does not attach a causal-LM
        head or a causal attention mask. Asserts trainable parameter count > 0
        after wrapping to catch silent target_modules mismatches.
        """
        if self.model is None:
            raise RuntimeError("ESM-2 base model is not loaded; cannot attach LoRA.")
        try:
        ...
        # NOTE: do NOT pre-set ``param.requires_grad = True`` on the base
        # model. ``get_peft_model`` freezes the base and unfreezes only the
        # injected LoRA adapter parameters; eagerly unfreezing first turns
        # LoRA into full fine-tuning.
        ...
        logger.info(
            ...
            lora_rank, target_modules, trainable, total, 100.0 * trainable / max(total, 1),
        )
