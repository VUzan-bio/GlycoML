...
class ESMEmbedder:
    """Embed protein sequences using ESM2 models."""

    def __init__(self, model_name: str, cache_dir: Path, device: torch.device, max_len: int = 1024) -> None:
        if esm is None:
            raise ImportError("fair-esm is required for ESM embeddings.")
        ...
        self.layer = max(self.model.num_layers, 1)
    ...
    def _cache_path(self, sequence: str) -> Path:
        return self.cache_dir / f"{_hash_text(sequence)}.pt"

    def embed(self, sequence: str) -> torch.Tensor:
        if not sequence:
            return torch.zeros((1, self.model.embed_dim), dtype=torch.float32)
        ...
        reps = results["representations"][self.layer][0, 1 : len(sequence) + 1].cpu()
