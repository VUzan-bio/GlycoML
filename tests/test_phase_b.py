...
# Load glycan_encoder.py directly via its file path so that the package
# __init__.py chain (which pulls heavy optional deps) does not need to import.
_GE_PATH = SRC_ROOT / "glycoml" / "phase2" / "models" / "glycan_encoder.py"
_spec = importlib.util.spec_from_file_location("glycan_encoder_direct", _GE_PATH)
glycan_encoder = importlib.util.module_from_spec(_spec)
sys.modules["glycan_encoder_direct"] = glycan_encoder
_spec.loader.exec_module(glycan_encoder)

tokenize_iupac = glycan_encoder.tokenize_iupac
count_monosaccharides = glycan_encoder.count_monosaccharides
COMMON_MONOSACCHARIDES = glycan_encoder.MONOSAC_TYPES
